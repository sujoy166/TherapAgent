"""PATH training loop — AdamW + grad clip + early stopping (paper §4.3)."""
...
from .model import PathGraphTransformer, weighted_bce_with_logits


def _epoch(model, loader, pos_weight, optim=None, device="cpu", clip=None):
...
def fit(model: PathGraphTransformer, loaders: dict, pos_weight, cfg,
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        tr = _epoch(model, loaders["train"], pw, optim=optim,
                    device=device, clip=cfg.grad_clip)
        with torch.no_grad():
