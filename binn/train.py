"""Training loop with early stopping + plateau LR scheduler."""
...
from .model import BINN, weighted_bce


def _epoch(model: BINN, loader, pos_weight, optim=None, device="cpu") -> Dict[str, float]:
    ...
    for xb, yb in loader:
        ...
        out = model(xb)
        loss = weighted_bce(out["prob"], yb, pos_weight)
        if train_mode:
            optim.zero_grad()
            loss.backward()
            optim.step()
...
def fit(model: BINN, loaders: dict, pos_weight, cfg, device: str = "cpu",
